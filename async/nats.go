...
func (n *natsMessaging) queueSubscribeJetStream(ctx context.Context, topic string, _ string, callback MessageHandler) error {
	...
	for {
		...
		for msg := range mc.Messages() {
			...
			err := msg.InProgress()
			if err != nil {
				log.Errorf("Error marking message as in progress: %v", err)
			}
